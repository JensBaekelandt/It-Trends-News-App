...
const translations = {
  en: {
    ...
    emptyText: "Save articles to read later. Your bookmarks will appear here.",
    emptyCta: "Explore"
  },
  nl: {
    navFeed: "Feed",
    ...
    emptyText: "Sla artikels op om later te lezen. Je bladwijzers verschijnen hier.",
    emptyCta: "Verkennen"
  }
};
...
const pagerBlock = document.getElementById("pagerBlock");
const emptyTitleEl = document.getElementById("emptyTitle");
const emptyTextEl = document.getElementById("emptyText");
const emptyCtaTextEl = document.getElementById("emptyCtaText");
...
function showEmpty(isEmpty) {
  ...
  if (bookmarksContainer) bookmarksContainer.classList.toggle("hidden", isEmpty);
  if (sortBlock) sortBlock.classList.toggle("hidden", isEmpty);
  if (pagerBlock) pagerBlock.classList.toggle("hidden", isEmpty);
}
...
function applyTranslations() {
  ...
  if (emptyCtaTextEl) emptyCtaTextEl.textContent = t("emptyCta");
...
function renderBookmarks() {
  ...
  if (total === 0) {
    ...
    showingTextEl.textContent = format(t("showingText"), { from: 0, to: 0, total: 0 });
  ...
  showingTextEl.textContent = format(t("showingText"), { from, to, total });
...
function init() {
  ...
  if (savedLang) {
    ...
    if (normalized === "en" || normalized === "nl") state.lang = normalized;
  }
